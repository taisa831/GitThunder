...
Validate.prototype = {
	...
	isPostCode:function(string){
		hankakuRegExp = /^[0-9]{3}-[0-9]{4}$/
		return !hankakuRegExp.test(string);
	},
	/*--------------------------------------------------------------------------*/
	isPhone:function(string){
		phoneRegExp = /^\d{2,4}-\d{2,4}-\d{4}$/
	...
	check:function(){
		...
		for(var i=0;i<this.rules_array.length;i++){
			switch(this.rules_array[i].option){
				...
				/*--------------------------------------------------------------------------*/
				case'PostJapan':
					if (this.isPostCode(getValue(this.rules_array[i].id))){
						this.error_array.push(this.rules_array[i].error);
						this.e = false;
					}
				break;
				/*--------------------------------------------------------------------------*/
				case'Phone':
					if (this.isPhone(getValue(this.rules_array[i].id))){
						this.error_array.push(this.rules_array[i].error);
						this.e = false;
					}
				break;
